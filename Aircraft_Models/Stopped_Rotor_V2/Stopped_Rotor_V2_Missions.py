...
#   Full Mission Setup
# ------------------------------------------------------------------
def full_mission_setup_baseline(analyses,vehicle,simulated_days,flights_per_day,control_points,recharge_battery,topography_data): 
    ...
    for day in range(simulated_days):
        ...
        for f_idx in range(flights_per_day):
            ...
            segment.analyses.extend( analyses.base )                  
            segment.altitude                                 = 700.0 * Units.ft  
            segment.air_speed                                = 125.   * Units['mph']
            cruise_distance                                  = topography_data.flight_range - 30.19 * Units.nmi
            ...
            segment.state.unknowns.throttle                 = 0.4 *  ones_row(1)  
            segment.process.iterate.unknowns.mission        = SUAVE.Methods.skip
            segment.process.iterate.conditions.stability    = SUAVE.Methods.skip
            segment.process.finalize.post_process.stability = SUAVE.Methods.skip   
            segment = vehicle.networks.lift_cruise.add_transition_unknowns_and_residuals_to_segment(segment,\
...
#   Full Mission Setup
# ------------------------------------------------------------------
def high_altitude_constant_elevation_cruise(analyses,vehicle,simulated_days,flights_per_day,control_points,recharge_battery,topography_data): 
    ...
    for day in range(simulated_days):
        ...
        for f_idx in range(flights_per_day):
            ...
            segment.tag = "Vertical_Climb"  + "_F_" + str(flight_no) + "_D" + str (day) 
            segment.analyses.extend( analyses.base )  
            segment.altitude_start                          = 0.0  * Units.ft  + topography_data.departure_location[2]
            segment.altitude_end                            = 100.  * Units.ft  + topography_data.departure_location[2]
            segment.climb_rate                              = 500. * Units['ft/min'] 
            segment.true_course                             = topography_data.true_course    
            if day == 0:        
                segment.battery_energy                      = vehicle.networks.lift_cruise.battery.max_energy   
            ...
            segment.analyses.extend( analyses.transition ) 
            segment.altitude                                = 100.  * Units.ft     + topography_data.departure_location[2]              
            segment.air_speed_start                         = 500. * Units['ft/min']
            segment.air_speed_end                           = 0.8 * Vstall
            segment.acceleration                            = 2.5
            segment.pitch_initial                           = 0.0 * Units.degrees
            ...
            segment.analyses.extend( analyses.base )    
            segment.altitude_start                          = 100.0 * Units.ft    + topography_data.departure_location[2]
            segment.altitude_end                            = 500.0 * Units.ft   + topography_data.departure_location[2]
            segment.air_speed                               = 0.8   * Vstall
            segment.climb_angle                             = 2     * Units.degrees   
            segment.acceleration                            = 0.25   * Units['m/s/s']    
            segment.pitch_initial                           = 7.    * Units.degrees  
            ...
            segment.tag                                      = "Climb_1"   + "_F_" + str(flight_no) + "_D" + str (day) 
            segment.analyses.extend( analyses.base ) 
            segment.altitude_start                           = 500.0 * Units.ft  + topography_data.departure_location[2]
            segment.altitude_end                             = 1000. * Units.ft  + topography_data.departure_location[2]
            segment.climb_rate                               = 600.  * Units['ft/min']
            segment.air_speed_start                          = 95.   * Units['mph']
            segment.air_speed_end                            = 125.  * Units['mph']  
            segment.state.unknowns.throttle                  = 0.9   *  ones_row(1)    
            ...
            segment.tag                                      = "Climb_2" + "_F_" + str(flight_no) + "_D" + str (day) 
            segment.analyses.extend( analyses.base ) 
            segment.altitude_start                           = 1000.0 * Units.ft  + topography_data.departure_location[2]
            segment.altitude_end                             = 2500. * Units.ft + topography_data.departure_location[2]  
            segment.climb_rate                               = 500.  * Units['ft/min']
            segment.air_speed_start                          = 125.  * Units['mph']  
            segment.air_speed_end                            = 175.  * Units['mph']    
            segment.state.unknowns.throttle                  = 0.90    *  ones_row(1)  
            ...
            segment.tag                                      = "Constant_Altitude_Flight" + "_F_" + str(flight_no) + "_D" + str (day) 
            segment.analyses.extend( analyses.base )   
            segment.altitude_start                           = 2500.0 * Units.ft + topography_data.departure_location[2]  
            segment.altitude_end                             = 2500.0 * Units.ft   + topography_data.destination_location[2]  
            ...
            segment.tag = "Descent" + "_F_" + str(flight_no) + "_D" + str (day) 
            segment.analyses.extend( analyses.descent)  
            segment.altitude_start                           = 2500.0 * Units.ft   + topography_data.destination_location[2]
            segment.altitude_end                             = 1000. * Units.ft   + topography_data.destination_location[2]
            segment.climb_rate                               = -500.  * Units['ft/min']
            segment.air_speed_start                          = 175.  * Units['mph']  
            segment.air_speed_end                            = 1.1*Vstall    
            segment.state.unknowns.throttle                  = 0.8   *  ones_row(1)  
            ...
            segment.tag                                      = "Descent_Transition"  + "_F_" + str(flight_no) + "_D" + str (day) 
            segment.analyses.extend( analyses.base ) 
            segment.altitude_start                           = 1000.0 * Units.ft  + topography_data.destination_location[2]
            segment.altitude_end                             = 100.0 * Units.ft  + topography_data.destination_location[2]
            segment.air_speed                                = 1.1*Vstall 
            segment.climb_angle                              = 1 * Units.degrees
            segment.acceleration                             = -0.5 * Units['m/s/s']   
            segment.pitch_initial                            = 2. * Units.degrees     
            ...
            segment.tag                                     = "Decelerating_Transition"  + "_F_" + str(flight_no) + "_D" + str (day) 
            segment.analyses.extend( analyses.desceleration ) 
            segment.altitude                                = 100.  * Units.ft      + topography_data.destination_location[2]
            segment.air_speed_start                         = 100. * Units['mph']
            segment.air_speed_end                           = 300. * Units['ft/min'] 
            segment.acceleration                            = -1         
            segment.pitch_initial                           = 5.  * Units.degrees  
            ...
            segment.tag                                     = "Vertical_Descent" + "_F_" + str(flight_no) + "_D" + str (day) 
            segment.analyses.extend( analyses.base)     
            segment.altitude_start                          = 100.0  * Units.ft  + topography_data.destination_location[2]
            segment.altitude_end                            = 0.   * Units.ft  + topography_data.destination_location[2]
            segment.descent_rate                            = 300. * Units['ft/min']   
            segment.true_course                             = topography_data.true_course   
            segment.process.iterate.unknowns.mission        = SUAVE.Methods.skip
            segment.process.iterate.conditions.stability    = SUAVE.Methods.skip
...
#   Full Mission Setup
# ------------------------------------------------------------------
def medium_altitude_constant_elevation_cruise(analyses,vehicle,simulated_days,flights_per_day,control_points,recharge_battery,topography_data): 
    ...
    for day in range(simulated_days):
        ...
        for f_idx in range(flights_per_day):
            ...
            segment.altitude_end                            = 500.0 * Units.ft   + topography_data.departure_location[2]
            segment.air_speed                               = 0.8   * Vstall
            segment.climb_angle                             = 10     * Units.degrees   
            ...
            # ------------------------------------------------------------------ 
            segment                                          = Segments.Climb.Constant_Speed_Linear_Altitude(base_segment)
            segment.tag                                      = "Constant_Elevation_Cruise" + "_F_" + str(flight_no) + "_D" + str (day) 
            segment.analyses.extend( analyses.base )    
            segment.altitude_start                           = 1000.  * Units.ft + topography_data.departure_location[2]  
            segment.altitude_end                             = 1000.  * Units.ft   + topography_data.destination_location[2]  
            segment.air_speed                                = 150.   * Units['mph']
            cruise_distance                                  = topography_data.flight_range - 13.38* Units.nmi
            segment.distance                                 = cruise_distance  
            segment.state.unknowns.throttle                  = 0.8    *  ones_row(1)  
            segment.true_course                              = topography_data.true_course   
            segment = vehicle.networks.lift_cruise.add_cruise_unknowns_and_residuals_to_segment(segment)    
            ...
            segment.analyses.extend( analyses.base)  
            segment.altitude_start                           = 1000.0 * Units.ft   + topography_data.destination_location[2]
            segment.altitude_end                             = 500. * Units.ft   + topography_data.destination_location[2]
            ...
            segment.tag                                      = "Descent_Transition"  + "_F_" + str(flight_no) + "_D" + str (day) 
            segment.analyses.extend( analyses.base ) 
            segment.altitude_start                           = 500.0 * Units.ft  + topography_data.destination_location[2]
            segment.altitude_end                             = 100.0 * Units.ft  + topography_data.destination_location[2]
            segment.air_speed                                = 1.1*Vstall 
            ...
            segment.analyses.extend( analyses.desceleration ) 
            segment.altitude                                = 100.  * Units.ft      + topography_data.destination_location[2]
            segment.air_speed_start                         = 59.5 * Units['mph']
            segment.air_speed_end                           = 300. * Units['ft/min'] 
            segment.acceleration                            = -1         
            segment.pitch_initial                           = 5.  * Units.degrees  
            segment.pitch_final                             = 8. * Units.degrees   
...
#   Full Mission Setup
# ------------------------------------------------------------------
def low_altitude_constant_elevation_cruise(analyses,vehicle,simulated_days,flights_per_day,control_points,recharge_battery,topography_data): 
    ...
    for day in range(simulated_days):
        ...
        for f_idx in range(flights_per_day):
            ...
            segment.tag                                      = "Constant_Elevation_Cruise" + "_F_" + str(flight_no) + "_D" + str (day) 
            segment.analyses.extend( analyses.base )    
            segment.altitude_start                           = 500.  * Units.ft + topography_data.departure_location[2]  
            segment.altitude_end                             = 500.  * Units.ft   + topography_data.destination_location[2] 
            segment.state.numerics.number_control_points     = control_points*3
            segment.air_speed                                = 121.   * Units['mph']
            cruise_distance                                  = topography_data.flight_range - 2.13* Units.nmi
            segment.distance                                 = cruise_distance  
            segment.state.unknowns.throttle                  = 0.8    *  ones_row(1)  
            segment.true_course                              = topography_data.true_course   
            segment = vehicle.networks.lift_cruise.add_cruise_unknowns_and_residuals_to_segment(segment)    
...
# ------------------------------------------------------------------
def approach_departure_mission_setup(analyses,vehicle,control_points,flight_data):
    ...
    segment.climb_rate                              = 500. * Units['ft/min']  
    segment.battery_pack_temperature                = atmo_data.temperature[0,0]    
    segment.true_course                             = flight_data.true_course              
    segment.process.iterate.unknowns.mission        = SUAVE.Methods.skip
    segment.process.iterate.conditions.stability    = SUAVE.Methods.skip
    segment.process.finalize.post_process.stability = SUAVE.Methods.skip
    segment = vehicle.networks.lift_cruise.add_lift_unknowns_and_residuals_to_segment(segment,\
