...
def main(): 
    ...
    max_y             = 2.5*Units.nmi   # maxiumum y (lateral) coordinate of acoustic computational domain
    ...
    for ang in range(len(true_course_angles)): 
        Q_idx             = 1        
        true_course_angle = true_course_angles[ang]* Units.degrees
        for i in range(len(X_LIM)-1):
            for j in range(len(Y_LIM)-1): 
                # start simulation clock
                ti                = time.time() 
                
                print('Running Quardant:' + str(Q_idx) + ' Angle: ' + str(true_course_angles[ang]) ) 
                min_x = X_LIM[i]
                max_x = X_LIM[i+1]
                min_y = Y_LIM[j]
                max_y = Y_LIM[j+1]
                ...
                # SAVE RESULTS
                filename          = 'SR_V2_TG_Noise_Angle_' + str(true_course_angles[ang]) + '_Q' + str(Q_idx)+ '_Nx' + str(N_gm_x) + '_Ny' + str(N_gm_y)
    ...
    if plot_mission: 
        Stopped_Rotor_V2.Stopped_Rotor_V2_Plots.plot_results(noise_results,run_noise_model)       
